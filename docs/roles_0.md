...
- [x] Read `app.py` (`load_data`, `build_event_impacts`, `case_facts`,
...
- [x] Confirm the `AlignmentReport` JSON contract in PRD §4.4 is sufficient;
      if not, propose a change to the PRD before diverging.

Assumption recorded during implementation: PRD §4.3 defines four alignment
dimensions, so the report and UI implement all four despite the acceptance
criteria's shorthand reference to three dimensions.

## Phase 1 — Deterministic fact packet

- [x] Create `src/singhacks26/alignment.py` with a
...
- [x] Include `allowed_evidence_ids` in the packet (note section names, data
      file names, event ids/dates) and mark `synthetic_data: True`.
- [x] Add unit tests for the fact packet builder against real `data/` rows
      (`tests/test_alignment.py`); assert no uncensored name/actual age leaks
      into the packet.

## Phase 2 — LLM alignment analysis

- [x] In `alignment.py`, add `analyze_alignment(data, client_id, vault_text)`
      that calls the OpenAI API the same way as `ai_brief.draft_ai_brief`
      (`responses.parse`, pydantic output, reads `.env`).
- [x] Use pydantic models for `AlignmentReport`, `Conflict`, and dimensions
      matching PRD §4.4 exactly (fields, severities, categories).
- [x] Reuse `ai_brief` validation patterns: reject any number the model
...
- [x] Treat `event_log.csv` as authoritative ground truth: the
      event-consistency dimension may only cite events present in the file.
- [x] Fail loudly if `OPENAI_API_KEY` is missing (feature-level error), per
      PRD §2 / §8.

## Phase 3 — AlignmentStore cache

- [x] Add an `AlignmentStore` class in `alignment.py` mirroring
...
- [x] Add a compact alignment/conflict panel or tab inside the Client deep
      dive page (PRD §6): band metric, dimension statuses, top conflicts.
- [x] Where applicable, add the client's worst conflict to the "Attention
      map" problem list so conflicts reach the RM's prioritised attention.
- [x] Keep UI copy in the existing style/voice and all rendering in `app.py`
